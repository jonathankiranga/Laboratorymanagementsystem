      ...
      <table id="resultsGrid" class="table table-bordered table-striped">
        <thead>
          <tr>
            ...
            <th>
                Action 
                <span id="saveSpinner" style="display:none; margin-left:10px; color: #007bff;">
...
// Bind filter & global result type
function bindFilter() {
...
function loadGrid(page = 1) {
    ...
    const category     = localStorage.getItem('category') || 'microbiological';
...
function renderRow(row) {
    ...
    const $sel = $('<select>', { class: 'result-type form-control' }).html(`
...
function showRowEditor($tr, type) {
    ...
    let $input;
//// 'ND','Absent','Detected','Below Limit','Detected Range','Trace','Above Limit','Inconclusive','Error','Invalid'
    if (type === 'quantitativeField') {
        $input = $('<input>', { type: 'number', class: 'result-value form-control', placeholder: 'Enter value' });
    } else if (type === 'qualitativeField') {
        $input = $('<select>', { class: 'result-value form-control' }).html(`
        ...
        $input = $('<input>', { type: 'text', class: 'result-value form-control', placeholder: 'e.g. <2 or 1-5' });
    }
...
function prepareSave($tr) {
    ...
    if (!resultType || !resultValue) {
        alert('Please select a result type and enter a value.');
        return;
    }
...
function saveResult(dataObj) {
    $('#saveSpinner').show();

    // Collect extra context from localStorage if needed
    ...
    $.ajax({
        ...
        data: JSON.stringify(payload), // Full JSON conversion
        dataType: 'json',
        success: function(res) {
            $('#saveSpinner').hide();
            if (res.success) {
                const $row = $(`tr[data-resultsid="${dataObj.resultsid}"]`);
                $row.css('background-color', '#d4edda').addClass('saved-row');
...
function renderPagination(totalPages, page) {
    ...
    const makeBtn = (label, target, disabled = false, active = false) => {
        return $('<button>', { text: label, disabled: disabled, class: active ? 'active' : '' })
            .on('click', () => loadGrid(target));
    };

    $container.append(makeBtn('«', Math.max(1, page - 1), page === 1));
    for (let i = 1; i <= totalPages; i++) {
        if(i === 1 || i === totalPages || (i >= page - 2 && i <= page + 2)) {
            $container.append(makeBtn(i, i, false, i === page));
        }
    }
    $container.append(makeBtn('»', Math.min(totalPages, page + 1), page === totalPages));
}
